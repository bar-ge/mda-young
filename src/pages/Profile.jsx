  ...
  return (
    <div className="flex flex-col gap-4 pt-3">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col items-center gap-3 text-center">
        ...
        <span className={`text-xs font-semibold px-3 py-1 rounded-full ${roleColors[profile?.role] || roleColors.volunteer}`}>
          {roleLabels[profile?.role] || 'מתנדב'}
        </span>
      </div>
      ...
      {editing ? (
        <form onSubmit={save} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 text-right">שם מלא</label>
            <input
              value={form.full_name}
              onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))}
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#E30613]/30 focus:border-[#E30613] text-right"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 text-right">טלפון</label>
            <input
              type="tel"
              dir="ltr"
              value={form.phone}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {menuItems.map((item, i) => (
          <button
            key={i}
            className={`w-full px-5 py-4 flex items-center gap-3 text-right active:bg-gray-50 transition-colors ${
              i < menuItems.length - 1 ? 'border-b border-gray-50' : ''
            }`}
          >
            <svg className="w-4 h-4 text-gray-300 mr-auto" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
      ...
      <p className="text-center text-xs text-gray-300 pb-2">מד״א צעירים — פלטפורמת מתנדבים v1.0</p>
    </div>
  )
}
