...
function DateField({ value, onChange, min, max }) {
  ...
  function handleTextChange(e) {
    ...
    if (digits.length === 8) {
      ...
      if (!isNaN(new Date(iso + 'T12:00:00'))) onChange({ target: { value: iso } })
    } else if (!digits) {
  ...
  function handleCalendarChange(e) {
    onChange(e)
    setRaw(toDisplay(e.target.value))
  }

  const inputCls = 'w-full px-3 py-2 pl-9 rounded-xl border border-gray-200 text-sm text-gray-800 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#E30613]/20 focus:border-[#E30613] transition-all'

  return (
    <div className="relative w-full">
      {/* Visible text input — always editable */}
      <input
        type="text"
        value={raw}
        ...
        dir="ltr"
      />
      {/* Calendar-icon area: hidden date input + icon on top */}
      <div className="absolute left-0 top-0 w-9 h-full flex items-center justify-center">
        <input
          ref={calRef}
          type="date"
          value={value || ''}
          ...
          tabIndex={-1}
        />
        <svg className="w-3.5 h-3.5 text-gray-400 pointer-events-none shrink-0" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
