...
export default function DutyVehicles() {
  ...
  const [section, setSection] = useState('vehicles') // vehicles | hours | assign
  ...
  async function loadAll() {
    ...
    const [{ data: veh }, { data: sh }, { data: drv }] = await Promise.all([
      ...
      supabase.from('duty_shifts')
        .select('*, duty_vehicles(name,plate,vehicle_type), driver:profiles!duty_shifts_driver_id_fkey(id,full_name)')
  ...
  async function saveShift(e) {
    ...
    setSaving(true)
    const startTs = `${sDate}T${sStart}:00`
    const endTs   = `${sDate}T${sEnd}:00`
    const { error } = await supabase.from('duty_shifts').insert({
      vehicle_id: sVehicle, start_time: startTs, end_time: endTs, notes: sNotes.trim() || null,
    })
    setSaving(false)
        ...
        <>
          ...
          {section === 'vehicles' && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {vehicles.map(v => (
                  <div key={v.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3.5 flex items-center gap-3">
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => startEdit(v)}
                        className="text-xs text-blue-500 hover:text-blue-700 font-medium transition-colors">
                        עריכה
                      </button>
                      <button onClick={() => deactivateVehicle(v.id, v.name)}
          ...
          {section === 'hours' && (
            <div className="flex flex-col gap-4">
              {/* Add shift form */}
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                <p className="text-sm font-bold text-gray-700 text-right mb-3">הוספת כוננות</p>
              ...
              <div className="flex flex-col gap-2">
                ...
                {shifts.length === 0 && (
                  <p className="text-gray-400 text-sm text-center py-4">אין כוננויות קרובות</p>
                )}
                {shifts.map(s => (
                  <div key={s.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 text-right">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {formatDate(s.start_time)} · {formatHour(s.start_time)}–{formatHour(s.end_time)}
                      </p>
                    </div>
          ...
          {section === 'assign' && (
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                ...
                <form onSubmit={assignDriver} className="flex flex-col gap-2.5">
                  <select value={aShift} onChange={e=>setAShift(e.target.value)} required className={SELECT}>
                    ...
                    {shifts.filter(s => s.status !== 'cancelled').map(s => (
              ...
              <div className="flex flex-col gap-2">
                ...
                {shifts.length === 0 && (
                  <p className="text-gray-400 text-sm text-center py-4">אין כוננויות</p>
                )}
                {shifts.map(s => (
                  <div key={s.id} className={`bg-white rounded-2xl border shadow-sm p-3.5 flex items-center gap-3 ${
                    ...
                    {s.driver && (
                      <button onClick={() => unassignDriver(s.id)}
                        className="text-xs text-gray-400 hover:text-red-500 font-medium transition-colors shrink-0">
                        הסר
                      </button>
                    )}
