...
export default function Dispatcher() {
  ...
  async function load() {
    ...
    const merged = rows.map(a => ({ ...a, profile: profileMap[a.user_id] }))
    setAssignments(merged)

    if (filter === 'pending') setPendingCount(merged.length)
          ...
          >
            ...
            {pendingCount > 0 && (
              <span className={`w-4 h-4 rounded-full text-[10px] flex items-center justify-center font-black ${
                filter === 'pending' ? 'bg-white text-[#E30613]' : 'bg-[#E30613] text-white'
              }`}>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-14 text-center">
          ...
          <p className="text-gray-400 text-sm">
            {filter === 'pending' ? 'אין רישומים ממתינים לאישור' : 'אין רישומים שטופלו'}
          </p>
        </div>
      ) : (
              ...
              <div key={a.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col gap-3">
                ...
                <div className="flex items-center justify-end gap-3">
                  <div className="text-right">
                    ...
                    {profile?.phone && (
                      <a href={`tel:${profile.phone}`} className="text-xs text-[#E30613] font-medium">
                        {profile.phone}
                      </a>
                    )}
                  </div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {formatShortDate(shift.start_time)} · {formatHour(shift.start_time)}–{formatHour(shift.end_time)}
